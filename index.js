...
function copy(originalFlavors) {
    const copy = originalFlavors.slice();
    return copy
} 
...
const isTrue = is31Flavors(originalFlavors)

function is31Flavors(originalFlavors){
    
    const answer = Boolean(originalFlavors.length == 31)
    return answer;
}
...
function removeFlavorByName(arr, string){
    const newArray = arr.indexOf(string);
    arr.splice(newArray, 1)
    return arr
}
